...
int main()
{
	...
	for(x=1;x<=T;x++)
	{
		...
		for(i=0;i<k;i++)
		{
			for(j=i;j<k;j++)
			{
				if(lexicographical_compare(player[i],player[i]+strlen(player[i]),player[j],player[j]+strlen(player[j])))
				{
				}
				else
				{
